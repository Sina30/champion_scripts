...
AddEventHandler('skinchanger:modelLoaded', function()
	ESX.TriggerServerCallback('champion_tattos:GetPlayerTattoos', function(tattooList)
		if tattooList then
...
function BuyTattoo(collection, name, label, price)
	ESX.TriggerServerCallback('champion_tattos:PurchaseTattoo', function(success)
		if success then
...
function RemoveTattoo(name, label)
	...
	TriggerServerEvent("champion_tattos:RemoveTattoo", currentTattoos)
	ESX.ShowNotification("Du hast das Tattoo" .. GetLabelText(label) .. "lasern lassen")
end
...
function CreateScale(sType)
	...
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Sicht ändern", 21, "Change Opacity", {90, 89}, "Kaufen/Lasern Tattoo", 191)
		scaleString = sType
	end
end
