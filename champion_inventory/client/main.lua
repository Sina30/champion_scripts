...
function toggleField(enable)
    ...
    ESX.TriggerServerCallback('champion_inventory:loadmoney', function(money)
        if enable then
            SendNUIMessage({
                action = 'open',
                money = money
...
RegisterNetEvent('champion_inventory:openWithXMenu')
AddEventHandler('champion_inventory:openWithXMenu', function(max)
    SendNUIMessage({
        action = 'open',
        money = money
    })
...
function ReloadInventory()
    ESX.TriggerServerCallback('champion_inventory:loadmoney', function(money)
        ...
        ESX.TriggerServerCallback('champion_inventory:loadTarget', function(data)
...
function Waffen()
    ESX.TriggerServerCallback('champion_inventory:loadmoney', function(money)
        ...
        ESX.TriggerServerCallback('champion_inventory:loadTargetWeapons', function(data)
            for key, value in pairs(data) do
                SendNUIMessage({
                    action = "addw",
                    identifier = value.identifier,
...
RegisterNUICallback('use', function(data, cb)
    ...
    for i = 1, tonumber(data.amount), 1 do
        TriggerServerEvent('champion_inventory:useItem', data.item)
        Wait(500)
    end
...
RegisterNUICallback('throw', function(data, cb)
    ...
    TriggerServerEvent('champion_inventory:throwItem', data.item, tonumber(data.amount))
...
RegisterNUICallback('throwweapon', function(data, cb)
	...
	Citizen.Wait(1300)
	ClearPedTasksImmediately(playerPed) 
    TriggerServerEvent('champion_inventory:throwWeapon', data.item, tonumber(data.amount))
    ESX.ShowNotification(("Du wirfst %sx %s weg"):format(data.amount, data.label))
    cb('ok')
end)
...
RegisterNUICallback('give', function(data, cb)
    ...
    if player == -1 or dist > 3.0 then
        ...
        Citizen.Wait(1300)
        ClearPedTasksImmediately(playerPed)
        TriggerServerEvent('champion_inventory:giveItem', data.item, data.amount, GetPlayerServerId(player))
...
RegisterNUICallback('giveweapon', function(data, cb)
    ...
    if player == -1 or dist > 3.0 then
        ...
        Citizen.Wait(1300)
        ClearPedTasksImmediately(playerPed)
        TriggerServerEvent('champion_inventory:giveWeapon', data.item, data.amount, GetPlayerServerId(player))
        ESX.ShowNotification(("Du hast jemanden %sx %s zugesteckt"):format(data.amount, data.label))
    end
...
RegisterNUICallback('throwCash', function(data, cb)
    toggleField(false)
    SetNuiFocus(false, false)
    TriggerServerEvent('champion_inventory:throwCash', tonumber(data.amount))
...
RegisterNUICallback('givecash', function(data, cb)
    ...
    if player == -1 or dist > 3.0 then
        ...
        Citizen.Wait(1300)
        ClearPedTasksImmediately(playerPed)
        TriggerServerEvent('champion_inventory:giveCash', tonumber(data.amount), GetPlayerServerId(player))
        ESX.ShowNotification(('Du hast jemanden %s$ zugesteckt'):format(data.amount))
    end
end)

RegisterNetEvent('champion_inventory:setMax')
AddEventHandler('champion_inventory:setMax', function(max)
    SendNUIMessage({
        action = 'updatemax',
        max = max
    })
