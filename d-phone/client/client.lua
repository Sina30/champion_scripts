...
RegisterNUICallback("loadprofilapp", function(data, cb)
	ESX.TriggerServerCallback('d-phone:loadprofilapp', function(cb) 
		SendNUIMessage({action="updateprofilapp", vorname=cb.vorname, nachname=cb.nachname, nummer=cb.nummer, fraktion=cb.fraktion, fraktionr=cb.fraktionr, gwd=cb.gwd})
	end)
end)
...
AddEventHandler("d-phone:client:sharecontact", function(name, number)
	...
	TriggerEvent("champion:notify", "Handy - System", _U("sharecontact"), "green", 10000)
end)
...
AddEventHandler("d-phone:client:acceptsharecontact", function()
	if sharependingrequest == true then
		...
		TriggerEvent("champion:notify", "Handy - System", _U("sharecontactaccepted"), "white", 3500)
	else
		TriggerEvent("champion:notify", "Handy - System", _U("nosharecontact"), "white", 3500)
	end
end)
...
AddEventHandler("d-phone:client:declinesharecontact", function()
	if sharependingrequest == true then
		...
		TriggerEvent("champion:notify", "Handy - System",  _U("sharecontactdeclined"), "white", 3500)
	else
		TriggerEvent("champion:notify", "Handy - System",  _U("nosharecontact"), "white", 3500)
	end
end)
			...
			if incall == true and phoneopen == true then 
				...
				if isprogess == false then
					...
					if hasphone == true then
						...
						if hasnumber2 == false then
							TriggerServerEvent("d-phone:server:checkphonenumber2", GetPlayerServerId(PlayerId()))
						end
						Wait(100)
						...
						isprogess = false
						--TriggerEvent("champion:notify", _U("needphone"))
						TriggerEvent("champion:notify", "HANDY", "Du brauchst ein Handy!", "red", 2500)
					end
				end
			end
		end
...
RegisterNUICallback("setPosMD", function(data, cb)
	TriggerEvent("d-phone:client:setPosMD", data.freq)
end)

RegisterNetEvent("d-phone:client:setPosMD")
AddEventHandler("d-phone:client:setPosMD", function()
	TriggerEvent("champion:notify", "Handy - System", "Markierung erfolgreich gesetzt", "green", 3500)
	SetNewWaypoint(303.84063720703, -586.31127929688)
end)

RegisterNUICallback("setPosPD", function(data, cb)
	TriggerEvent("d-phone:client:setPosPD", data.freq)
end)

RegisterNUICallback("setPosCasino", function(data, cb)
	TriggerEvent("d-phone:client:setPosCasino", data.freq)
end)

RegisterNetEvent("d-phone:client:setPosCasino")
AddEventHandler("d-phone:client:setPosCasino", function()
	TriggerEvent("champion:notify", "Handy - System", "Markierung erfolgreich gesetzt", "green", 3500)
	SetNewWaypoint(916.62, 51.32)
end)

RegisterNetEvent("d-phone:client:setPosPD")
AddEventHandler("d-phone:client:setPosPD", function()
	TriggerEvent("champion:notify", "Handy - System", "Markierung erfolgreich gesetzt", "green", 3500)
	SetNewWaypoint(425.130, -979.558)

end)

RegisterNUICallback("setPosSchlepp", function(data, cb)
	TriggerEvent("d-phone:client:setPosSchlepp", data.freq)
end)

RegisterNetEvent("d-phone:client:setPosSchlepp")
AddEventHandler("d-phone:client:setPosSchlepp", function()
	TriggerEvent("champion:notify", "Handy - System", "Markierung erfolgreich gesetzt", "green", 3500)
	SetNewWaypoint(408.1, -1625.15)

end)

RegisterNUICallback("setPosLSC", function(data, cb)
	TriggerEvent("d-phone:client:setPosLSC", data.freq)
end)

RegisterNetEvent("d-phone:client:setPosLSC")
AddEventHandler("d-phone:client:setPosLSC", function()
	TriggerEvent("champion:notify", "Handy - System", "Markierung erfolgreich gesetzt", "green", 3500)
	SetNewWaypoint(-199.11, -1315.35)
...
RegisterNUICallback("setgps", function(data, cb)
	...
	SetNewWaypoint(x, y)
	TriggerEvent("champion:notify", "Handy - System", _U("waypointset"), "green", 3500)
end)
...
RegisterNUICallback("changewallpaperOne", function(data, cb)
	local url = data.url

	TriggerServerEvent("d-phone:server:changewallpaper", GetPlayerServerId(PlayerId()), "https://cdn.discordapp.com/attachments/852939253252227172/874333880709181460/img.png")
end)

RegisterNUICallback("changewallpaperTwo", function(data, cb)
	local url = data.url

	TriggerServerEvent("d-phone:server:changewallpaper", GetPlayerServerId(PlayerId()), "https://cdn.discordapp.com/attachments/852939253252227172/874336057192251465/img2.png")
end)
...
AddEventHandler("d-phone:client:callrequest", function(requester, requesternumber, sourcenumber, contact)
	if incall == false and flightmode == false then 
		...
		TriggerServerEvent("champion:notify", requester, "Handy - System", _U("cantcall"), "red", 5000)
...
AddEventHandler("d-phone:client:setradio", function(freq)
	...
	if canaccess == true then
		...
		TriggerEvent("champion:notify", "Handy - System", _U("setfreq", freq), "green", 3500)
	elseif  frq >= 17 then
		if Config.MumbleVoip == true then
			exports["mumble-voip"]:SetRadioChannel(frq)
		elseif Config.TokoVoip == true then
		...
		TriggerEvent("champion:notify", "Handy - System", _U("cantsetfreq"), "white", 3500)
	end
end)
...
AddEventHandler("d-phone:client:removeradio", function()
	...
	TriggerEvent("champion:notify", "Handy - System", _U("leftfreq"), "red", 3500)
end)
...
function garageCarsLoad()
    ESX.TriggerServerCallback('champion_garage:GetGarageVehicles', function(Cars)
        ...
        ESX.TriggerServerCallback('champion_garage:GetNearVehiclesByOwner', function() 
