...
RegisterNetEvent("champion_carmenu:checkCuff")
AddEventHandler("champion_carmenu:checkCuff", function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if distance~=-1 and distance<=3.0 then
        ESX.TriggerServerCallback("champion_carmenu:isCuffed",function(cuffed)
            if not cuffed then
                TaskPlayAnim(PlayerPedId(), "mp_arrest_paired", "cop_p2_back_right", 8.0, -8, 4000, 48, 0, 0, 0, 0) 
                TriggerServerEvent("champion_carmenu:randa",GetPlayerServerId(player),true)
            else
                TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'a_uncuff', 8.0, -8, 5000, 49, 0, 0, 0, 0)
                TriggerServerEvent("champion_carmenu:randa",GetPlayerServerId(player),false)
            end
...
RegisterNetEvent("champion_carmenu:wegda")
AddEventHandler("champion_carmenu:wegda",function()
    local player, distance = ESX.Game.GetClosestPlayer()
    if distance~=-1 and distance<=3.0 then
        TriggerServerEvent("champion_carmenu:wegda",GetPlayerServerId(player))
    else
        ESX.ShowNotification("~r~No players nearby")
    end
end)

RegisterNetEvent('champion_carmenu:forceUncuff')
AddEventHandler('champion_carmenu:forceUncuff',function()
    IsHandcuffed = false
...
RegisterNetEvent("champion_carmenu:randa")
AddEventHandler("champion_carmenu:randa",function()
    local playerPed = GetPlayerPed(-1)
