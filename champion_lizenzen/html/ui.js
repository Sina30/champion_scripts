$(document).ready(function(){
    window.addEventListener('message', function( event ) {            
      if (event.data.action == 'open') {
        ...
        for (let index = 0; index < event.data.pos.length; index++) {
          ...
          $('.content-elem[data-type="' + event.data.pos[index].type + '"] i').css("color", "grey");      
          $('.content-elem[data-type="' + event.data.pos[index].type + '"] span').css("color", "grey");  
        }
    ...
    $(document).keyup(function(e) {
      if (e.key === "Escape") {
        $('.container').css('display', 'none');
          $.post('http://champion_lizenzen/escape', JSON.stringify({}));
      }
    });
});