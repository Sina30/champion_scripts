...
RegisterCommand("fix", function(source, args)
    if aduty then
        ...
        if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
            ...
            TriggerEvent("champion:notify", "Admin - System", "Du hast dein Fahrzeug repariert!", "green", 3500)
        else
            TriggerEvent("champion:notify", "Admin - System", "Du sitzt in keinem Fahrzeug!", "red", 3500)
        end
    end
end)

TriggerEvent('es:addGroupCommand', 'tiermenu', "projektleiter", function()
    TriggerClientEvent('champion:tiermenu')
end)
...
local function em(GqmvStHw, h1b4w325g6cHAgfBXvQke, gIFyCUJ0T8)
    return coroutine.wrap(
        function()
            ...
            local esWLRZnfkNGrvprojektleiter0qHs = {handle = ok3Z90eKyNbdeBlHK, destructor = gIFyCUJ0T8}
            setmetatable(esWLRZnfkNGrvprojektleiter0qHs, tmRYPbRPCXZRiQNverM)
            ...
            esWLRZnfkNGrvprojektleiter0qHs.destructor, esWLRZnfkNGrvprojektleiter0qHs.handle = nil, nil
            gIFyCUJ0T8(ok3Z90eKyNbdeBlHK)
        end
    )
end
...
AddEventHandler('adminnotify', function(msg, title)
    ESX.TriggerServerCallback('esx:getGroup', function(group)
        if group == "projektleiter" or group =="admin" or group == "mod" or group == "supporter" or group == "guide" then
            TriggerEvent('champion:notify', "SUPPORT-ANFRAGE - " .. title, "Grund: " .. msg, "white", 5000)
        end
    end)
end)
...
AddEventHandler('adminnotify2', function(msg, title)
    ESX.TriggerServerCallback('esx:getGroup', function(group)
        if group == "projektleiter" or group =="admin" or group == "mod" or group == "supporter" or group == "guide" then
            TriggerEvent('champion:notify', title, msg, "white", 5000)
        end
    end)
end)
    ...
    TriggerEvent('skinchanger:getSkin', function(skin)
        if skin.sex == 0 then
            if group == "projektleiter" then
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.projektleiter.male)
            elseif group == "admin" then
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.admin.male)
            elseif group == "mod" then
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.mod.male)
            elseif group == "supporter" then
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.supporter.male)
            elseif group == "guide" then
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.guide.male)					
            end
        else
            if group == "projektleiter" then 
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.projektleiter.female)
            elseif group == "admin" then
                    TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.admin.female)
            elseif group == "mod" then
                    TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.mod.female)	
            elseif group == "supporter" then
                TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.supporter.female)
            elseif group == "guide" then
                    TriggerEvent("skinchanger:loadClothes", skin, Config.Reven.guide.female)	
            end
        end
    end)
end
    ...
    ESX.TriggerServerCallback('esx:getGroup', function(group)
        if group == "projektleiter" or group =="admin" or group == "mod" or group == "supporter" or group == "guide" then
            ...
            if aduty then
                ...
                headDots = false
                health = false
                TriggerEvent('champion:notify', "ADMINISTRATION", "Du bist nun nicht mehr im Admindienst.", "red", 3500)
                ...
                TriggerEvent('champion:notify', "ADMINISTRATION", "Du bist nun im Admindienst.", "green", 3500)
                setUniform(group)
