	...
	if not carry.InProgress then
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				TriggerServerEvent("okmachdas:sync",targetSrc)
		...
		TriggerServerEvent("okmachdas:stop",carry.targetSrc)
		carry.targetSrc = 0
	end
end,false)

RegisterNetEvent("okmachdas:syncTarget")
AddEventHandler("okmachdas:syncTarget", function(targetSrc)
...
RegisterNetEvent("okmachdas:cl_stop")
AddEventHandler("okmachdas:cl_stop", function()
	carry.InProgress = false
	ClearPedSecondaryTask(PlayerPedId())
	DetachEntity(PlayerPedId(), true, false)
end)
