...
RegisterNetEvent("rid_blackmarketcayo:sendShop")
AddEventHandler("rid_blackmarketcayo:sendShop", function(shopItems)
...
function OpenShopMenu(zone)
    ...
    TriggerServerEvent("rid_blackmarketcayo:stockUpdate")
    ...
    for i = 1, #Config.Zones[zone].Items, 1 do
        ...
        table.insert(elements, {
	        label = ("%s - <span style='color: red;'>%s</span> - <span style='color: orange;'>%s</span>"):format(item.label, _U("shop_menu_item", ESX.Math.GroupDigits(item.price)), item.stock),
	        price = item.price,
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "shop", {
        ...
        ESX.TriggerServerCallback("rid_blackmarketcayo:buyWeapon", function(bought)
            if bought then
...
AddEventHandler("rid_blackmarketcayo:hasEnteredMarker", function(zone)
...
AddEventHandler("rid_blackmarketcayo:hasExitedMarker", function(zone)
    CurrentAction = nil
    ESX.UI.Menu.CloseAll()
end)
    ...
    while true do
        ...
        if PlayerData.job and PlayerData.job.name ~= "police" then
            if isInMarker and not HasAlreadyEnteredMarker then
                ...
                TriggerEvent("rid_blackmarketcayo:hasEnteredMarker", currentZone)
            end
        end
        ...
        if not isInMarker and HasAlreadyEnteredMarker then
            ...
            TriggerEvent("rid_blackmarketcayo:hasExitedMarker", LastZone)
        end
        Citizen.Wait(wait)
    end
end)
