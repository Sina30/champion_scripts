...
ESX.RegisterServerCallback('rid_blackmarketcayo:getShop', function(source, cb)
	cb(shopItems)
end)

ESX.RegisterServerCallback('rid_blackmarketcayo:buyWeapon', function(source, cb, weaponName, zone)
	...
	if price == 0 then
		print(('rid_blackmarketcayo: %s attempted to buy a unknown weapon!'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.hasWeapon(weaponName) then
			...
			if stock == 0 or stock < 0 then
				print(('rid_blackmarketcayo: %s attempted to buy 0 stock weapon!'):format(xPlayer.identifier))
				...
				if zone == 'BlackMarket' then
					...
					if xPlayer.getAccount(money_type).money >= price then
						...
						-- Take stock logic
						MySQL.Async.execute('UPDATE black_marketcayo SET stock = stock - 1 WHERE item = @item', {
							['@item'] = weaponName
						}, nil)
...
function GetPrice(weaponName, zone)
	local price = MySQL.Sync.fetchScalar('SELECT price FROM black_marketcayo WHERE zone = @zone AND item = @item', {
		['@zone'] = zone,
		['@item'] = weaponName
	})
...
function GetStock(weaponName, zone)
	local stock = MySQL.Sync.fetchScalar('SELECT stock FROM black_marketcayo WHERE zone = @zone AND item = @item', {
		['@zone'] = zone,
		['@item'] = weaponName
	})
...
RegisterServerEvent('rid_blackmarketcayo:stockUpdate')
AddEventHandler('rid_blackmarketcayo:stockUpdate', function()

	-- Clear Current Table Inserts To prevent table stacking
	shopItems["BlackMarket"] = {}

	MySQL.Async.fetchAll('SELECT * FROM black_marketcayo', {}, function(result)
		for i=1, #result, 1 do
		...
		TriggerClientEvent('rid_blackmarketcayo:sendShop', -1, shopItems)
	end)
end)

Citizen.CreateThread(function()
    print("rid_blackmarketcayo: Started!")
...
function weaponRandomizer(d,h,m)
	...
	print("rid_blackmarketcayo: New Weapons in Black Market")
	...
	MySQL.Async.fetchAll('SELECT * FROM black_marketcayo', {}, function(result)
        for i=1, #result, 1 do
			...
			if current_stock == 0 then
				...
				MySQL.Async.execute('UPDATE black_marketcayo SET item = @item, stock = @stock, price = @price WHERE id = @id', {
					['@id'] = current_id,
					['@item'] = new_weapon,
					['@stock'] = new_stock,
					['@price'] = new_price
...
function weaponRestock()
	-- On Resource Restart
	print("rid_blackmarketcayo: Refilling all weapon stock")
    ...
    MySQL.Async.fetchAll('SELECT * FROM black_marketcayo', {}, function(result)
        for i=1, #result, 1 do
			...
			if current_stock == 0 then
				...
				-- Re-Stock Logic On Script Restart / Server Restart
				MySQL.Async.execute('UPDATE black_marketcayo SET stock = @stock WHERE item = @item', {
					['@item'] = current_weapon,
					['@stock'] = new_stock
				}, nil)
